...
import { createSessionsIndexHtml } from "./sessions-index.js";
import { createTranscriptPageHtml } from "./transcript-page.js";
import { transcriptDocument, transcriptFileName, transcriptMarkdown } from "./transcript.js";
import {
  bindHost,
  extraAllowedHosts,
  hostForUrl,
...
export async function serve({
  ...
  app.get("/", (_req, res) => {
    res.type("html").send(createSessionsIndexHtml());
  });

  // What the index page reads. Titles come from the artifact on disk rather than from state, so a
  // page renamed since it was opened lists under the name it has now.
    ...
    try {
      ...
      const listed = await Promise.all(
        sessions.map(async (session) => {
          ...
          const { title } = extractArtifactHead(html);
          return {
            key: session.key,
            title: title || path.basename(session.file),
            project: artifactProject(session.file),
            file: session.file,
            url: session.url || "",
  ...
  // The conversation as a document, and where its exports come from. Read-only: it takes nothing
  // out of the queue and changes no session state, so it is safe to open mid-review.
  app.get("/api/:key/transcript", async (req, res, next) => {
    try {
      ...
      const document = transcriptDocument(session);
      const download = req.query.download === "1";
      if (req.query.format === "markdown") {
        const { title } = extractArtifactHead(await readFile(session.file, "utf8").catch(() => ""));
        if (download) {
          res.setHeader("content-disposition", downloadDisposition(transcriptFileName(session.file, "md")));
        }
        res.type("text/markdown; charset=utf-8").send(transcriptMarkdown(document, { title }));
        return;
      }
      if (download) {
        res.setHeader("content-disposition", downloadDisposition(transcriptFileName(session.file, "json")));
      }
      res.json(document);
    } catch (error) {
      next(error);
  ...
  app.get("/session/:key/transcript", async (req, res, next) => {
    try {
      const session = await store.findByKey(req.params.key);
      if (!session) {
        sendSessionNotFound(req, res);
      ...
      const { title } = extractArtifactHead(await readFile(session.file, "utf8").catch(() => ""));
      res.setHeader("cache-control", "no-cache");
      res.type("html").send(createTranscriptPageHtml({ key: session.key, title, file: session.file }));
    } catch (error) {
      next(error);
    }
  });
  ...
  // The artifact on its own, with neither the review chrome around it nor the SDK inside it - the
  // index page's "Artifact only" link. It sits under /artifact/<key>/ so the page's relative assets
  // resolve through the asset route below, and it carries the same sandbox CSP as every other way
  // this server hands an artifact to a browser: an artifact must never run with this origin's
  // privileges, whoever asked for it.
  app.get(/^\/artifact\/([^/]+)\/source\.html$/, async (req, res, next) => {
    try {
      res.setHeader("content-security-policy", ARTIFACT_CONTENT_SECURITY_POLICY);
...
const chromeIcons = {
  ...
  // The sessions index: several pages, one of which is this one.
  sessions: chromeIcon(
    '<rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/>',
    14,
  ),
  // Desktop panel hide/show: the panel leaves to the right and comes back from it.
  chevronRight: chromeIcon('<polyline points="9 6 15 12 9 18"/>', 16, 2),
  chevronLeft: chromeIcon('<polyline points="15 6 9 12 15 18"/>', 16, 2),
};

// Which project a review belongs to, for the index: an artifact almost always lives in a
// `.lavish` directory inside the repo it is about, so the directory above that names it better
// than the artifact's own folder does.
function artifactProject(file) {
  const parts = String(file).split(path.sep);
  const lavishIndex = parts.lastIndexOf(".lavish");
  if (lavishIndex > 0) return parts[lavishIndex - 1];
