...
const BUBBLE_MORE_HTML = '<button class="bubble-more" type="button">Show more</button>';

function userBubbleTextHtml(entry, text) {
  const displayText = String(text || attachmentOnlyText(entry));
  if (!displayText) return "";
  const long = displayText.length > CHAT_CLAMP_CHARS;
  return (
...
function chatBubbleHtml(entry) {
  if (entry.role === "agent") {
    // A long reply folds the same way a long note does; the rendered Markdown block clamps as a
    // whole, so the toggle has one body to find whichever branch produced it.
    const long = String(entry.text || "").length > CHAT_CLAMP_CHARS;
    const clamp = long ? " is-clamped" : "";
    return (
      "<small>Agent</small>" +
      (typeof entry.html === "string" && entry.html
        ? '<div class="chat-md' + clamp + '">' + entry.html + "</div>"
        : '<div class="bubble-text' + clamp + '">' + escapeHtml(entry.text) + "</div>") +
      (long ? BUBBLE_MORE_HTML : "")
    );
  }
  return (
...
chatLog.addEventListener("click", (event) => {
  ...
  if (!toggle) return;
  const body = toggle.parentElement?.querySelector?.(".bubble-text, .chat-md");
  if (!body) return;
