...
export function chatEntryForPrompt(prompt, at) {
  ...
  // context blob - can say in `summary` what the human actually wrote, and that is what the
  // review's own transcript shows. The payload itself is unchanged: the agent still receives
  // `prompt`.
  const text = String(prompt.summary || "") || String(prompt.prompt || "");
