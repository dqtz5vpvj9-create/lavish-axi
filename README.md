...
- **Server upgrades** - One background server serves every session, so upgrading `lavish-axi` while reviews are open makes the next `lavish-axi <html-file>` replace that server. This fork converges the other open review pages on the new build rather than leaving them behind: each shows a banner reading "Lavish was updated. This page is running the previous version." while it waits, and picks the replacement up as soon as it answers. The exception is a page with unsent annotation text - that text is yours to finish, so it keeps the banner with its Check and reload and Dismiss buttons and catches up after your next send. Each connect also carries the build serving it, so a page that was asleep or offline through the replacement converges when it reconnects instead of sitting on old code. After `lavish-axi stop` nothing is coming to converge on: those pages say Lavish was stopped and to reload after you start it again, and stay where they are, and a restart that only picks up a local build says that rather than claiming an update. An open review page whose server simply goes away - idle self-shutdown, a crash, a machine that slept - shows that same banner once reconnecting has failed for a few seconds, rather than retrying a dead server silently while looking fine; it clears itself if the server comes back, and dismissing it keeps it dismissed until then.
