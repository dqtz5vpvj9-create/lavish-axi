...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  Sending turns each queued prompt into a chat bubble as its pill clears - annotations, whiteboard feedback and queued layout fixes included, each labelled with what it was about - so the panel keeps the whole conversation instead of only the messages that were typed into the composer. The transcript is stored with the session, so it survives a reload and appears in every open review of it. A long entry is folded to four lines with a **Show more** control. An artifact that composes its own agent payload - instructions, locations, quoted source - can add a `summary` to the prompt it queues, and the queue pill and the transcript then show that instead of the payload, while the agent still receives the payload unchanged.
  ...
  Composer feedback is visibly queued before the text box clears and stays queued until acknowledged, with persistent recovery guidance if sending fails or takes longer than 10 seconds.
...
- **Panel width and visibility** - At desktop widths the Conversation panel has a drag handle on its left edge: drag to resize it between 280px and 720px, double-click the handle to return to 360px, or focus it and use the arrow keys. The panel's heading carries a hide control, and Ctrl+\\ (Cmd+\\ on macOS) toggles it; while it is hidden the artifact takes the whole window and a Conversation button on the right edge brings it back. Both the width and the hidden state are this browser's preference and apply to every review it opens. Below 860px the phone sheet owns the layout and neither control appears.
...
- **Server upgrades** - One background server serves every session, so upgrading `lavish-axi` while reviews are open makes the next `lavish-axi <html-file>` replace that server. Every open review page then converges on the new build by itself: it shows a banner reading "Lavish was updated. This page is running the previous version." while it waits, and reloads as soon as the replacement answers. The exception is a page with unsent annotation text - that text is yours to finish, so it keeps the banner and its Check and reload button, and catches up on its own after your next send. Each connect also carries the build serving it, so a page that was asleep or offline through the replacement converges when it reconnects instead of sitting on old code. After `lavish-axi stop` nothing is coming to converge on: those pages say Lavish was stopped and to reload after you start it again, and stay where they are. A restart that only picks up a local build says that rather than claiming an update.
...
- **Reading a review back** - Every review's conversation is also a page of its own at `/session/<key>/transcript`, reachable from **Conversation history** in the review's overflow menu: the whole exchange in one column, filterable, and printable. It offers the same conversation as Markdown or JSON, by download or clipboard, and `lavish-axi transcript <html-file>` prints or writes the same Markdown from the terminal. Feedback that is queued but not yet sent is counted rather than included, because it is not part of the conversation until it is delivered.
- **Session index** - The server's own root URL (`http://127.0.0.1:4387/`, or the phone URL when Tailscale is up) lists every review it knows about, newest first, with the artifact's current title, its project, how long ago it changed, how much feedback is queued, and a filter. Each row opens the review or the artifact on its own; ended sessions are hidden behind a toggle. An **All sessions** button in a review's top bar opens it in a new tab, so one review always leads back to the rest.
- **Local-first state** - Session state stays under `~/.lavish-axi/` by default, or `LAVISH_AXI_STATE_DIR` when set.
- **Artifact state** - The artifact runs in a sandbox with no origin of its own, where `localStorage` throws on access rather than reading as empty - so a review page that keeps its own state (which item was decided, which sections are open) could not save it and usually said so in the page. Lavish now hands that page a working `localStorage`: it is seeded server-side with what the session already holds, and writes are mirrored through the review chrome into the session state, capped at 1 MiB and 200 keys per session. It follows the artifact rather than the browser, so the same decisions are there when the review is reopened from another device. `sessionStorage` is provided in memory only, since outliving the tab is the one thing it promises not to do. A standalone or exported copy is untouched and uses the browser's own storage.
...
| `lavish-axi history <html-file>`    | Re-read user feedback an earlier `poll` already delivered. Poll delivery is one-shot, so this is the recovery path when that output was truncated, filtered, or lost with the process that received it. Read-only: it consumes nothing, changes no session state, and does not start the server.                                                          |
| `lavish-axi transcript <html-file>` | Print the whole review conversation - every annotation, message and agent reply, oldest first - as Markdown. `--json` returns the structured form and `--out <path>` writes to a file. Read-only, and separate from `history`, which re-reads one delivered poll's agent-facing payload.                                                                  |
...
| `lavish-axi history`     | `--limit <n>`         | Return only the newest n delivered prompts.                                                                                                                                                                                         |
| `lavish-axi history`     | `--since <iso8601>`   | Return only prompts delivered at or after an ISO 8601 timestamp. A value Lavish cannot parse is refused rather than ignored.                                                                                                        |
