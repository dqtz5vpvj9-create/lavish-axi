...
test("every accepted prompt becomes a chat entry, not just typed messages", async () => {
  await withStore(async ({ store, session }) => {
    ...
    assert.deepEqual(
      stored.chat.map((entry) => [entry.text, entry.anchor?.excerpt || "", entry.kind || ""]),
      [
...
test("a page that composes its own payload can say what the human wrote", async () => {
  await withStore(async ({ store, session }) => {
    ...
    const stored = await store.findByKey(session.key);
    assert.deepEqual(
      stored.chat.map((entry) => [entry.text, entry.anchor?.excerpt]),
...
test("an image-only prompt is recorded with the label its pill showed", async () => {
  await withStore(async ({ store, session }) => {
    ...
    const stored = await store.findByKey(session.key);
    // The placeholder label itself moved to the chrome (`attachmentOnlyText`), which is where
    // the thumbnails are rendered; the store's job is to keep the entry at all rather than
    // dropping a prompt for carrying no text of its own.
    assert.deepEqual(
      stored.chat.map((entry) => [entry.kind, entry.text, entry.attachments?.length ?? 0]),
      [
        ["annotation", "", 1],
        ["message", "", 1],
      ],
